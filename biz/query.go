...
func Query(ctx context.Context) {
	...
	log.Info("query conditions got: %+v", user)
	...
	log.Info("subquery 1 got item: %+v", user)
	...
	log.Info("subquery 2 got item: %+v", user)
	...
	log.Info("select distinct got: %d", len(users))
	...
	log.Info("find with hints 2: (%d)%+v", len(users), users)
	...
	log.Info("FirstOrCreate got: %+v", user)
