...
func newCreditCard(db *gorm.DB, opts ...gen.DOOption) creditCard {
	...
	_creditCard.Bank = creditCardBelongsToBank{
...
type creditCardBelongsToBank struct {
	db *gorm.DB

	field.RelationField
}

func (a creditCardBelongsToBank) Where(conds ...field.Expr) *creditCardBelongsToBank {
	if len(conds) == 0 {
		return &a
	}
...
func (a creditCardBelongsToBank) WithContext(ctx context.Context) *creditCardBelongsToBank {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a creditCardBelongsToBank) Session(session *gorm.Session) *creditCardBelongsToBank {
	a.db = a.db.Session(session)
	return &a
}

func (a creditCardBelongsToBank) Model(m *model.CreditCard) *creditCardBelongsToBankTx {
	return &creditCardBelongsToBankTx{a.db.Model(m).Association(a.Name())}
}

type creditCardBelongsToBankTx struct{ tx *gorm.Association }

func (a creditCardBelongsToBankTx) Find() (result *model.Bank, err error) {
	return result, a.tx.Find(&result)
}

func (a creditCardBelongsToBankTx) Append(values ...*model.Bank) (err error) {
...
func (a creditCardBelongsToBankTx) Replace(values ...*model.Bank) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a creditCardBelongsToBankTx) Delete(values ...*model.Bank) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a creditCardBelongsToBankTx) Clear() error {
	return a.tx.Clear()
}

func (a creditCardBelongsToBankTx) Count() int64 {
	return a.tx.Count()
}
