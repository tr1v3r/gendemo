...
func newAddr(db *gorm.DB, opts ...gen.DOOption) addr {
	_addr := addr{}

	_addr.addrDo.UseDB(db, opts...)
	_addr.addrDo.UseModel(&model.Addr{})

	tableName := _addr.addrDo.TableName()
	_addr.ALL = field.NewAsterisk(tableName)
...
func (a addr) Columns(cols ...field.Expr) gen.Columns { return a.addrDo.Columns(cols...) }
...
func (a addr) clone(db *gorm.DB) addr {
	a.addrDo.ReplaceConnPool(db.Statement.ConnPool)
	return a
}

func (a addr) replaceDB(db *gorm.DB) addr {
...
func (a addrDo) Session(config *gorm.Session) *addrDo {
...
func (a addrDo) Delete(models ...*model.Addr) (result gen.ResultInfo, err error) {
